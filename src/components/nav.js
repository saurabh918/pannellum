...
export const Nav = () => {
  ...
  return (
    <div className='absolute bg-black top-0 left-20 shadow-md bg-opacity-60 h-[80px] flex justify-center w-[900px] rounded-b-lg z-10'>
      <h1 className='top-10 text-5xl text-white font-black flex justify-center self-center mr-10'>Pannelum</h1>
      <nav className='text-white text-2xl flex justify-center items-center'>
      {navItem.map((item,i) => (
          <Link className={`${navStyle} ${location.pathname === item.to && navStyleSelected}`} key={i} to={item.to}>{item.label}</Link>
        ))}
      </nav>
    </div>
  )
